...
PCG Biome actors can be spawned and configured, but **actual mesh generation requires BiomeTexture**
which can't be set via MCP tools yet. The PCG system shows `bGenerated=true` but
`GeneratedResources=[]` because the biome texture drives where meshes spawn.
